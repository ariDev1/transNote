...
class PanelSourceTests(unittest.TestCase):
    ...
    def test_recursive_deletion_has_canonical_root_containment_check(self):
        self.assertIn("function queueSafeRemove(rootDir, targetDir)", self.panel)
        self.assertIn("/usr/bin/realpath -m --", self.panel)
        self.assertIn('case "$target" in "$root"/*)', self.shell_view)
        self.assertIn('/usr/bin/rm -rf -- "$target"', self.shell_view)
