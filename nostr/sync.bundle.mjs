...
function schnorrSign(message, secretKey, auxRand = randomBytes(32)) {
  ...
  const t = Fn.toBytes(d ^ num(taggedHash("BIP0340/aux", a2)));
...
function chain(...args2) {
  ...
  const encode = args2.map((x) => x.encode).reduceRight(wrap, id);
  const decode2 = args2.map((x) => x.decode).reduce(wrap, id);
...
var AbstractSimplePool = class {
  ...
  subscribeMap(requests, params) {
    ...
    let handleEose = (i2) => {
      ...
      if (eosesReceived.filter((a2) => a2).length === groupedRequests.length) {
        params.oneose?.();
        handleEose = () => {
        };
      }
    ...
    let handleClose = (i2, url, reason) => {
      ...
      if (closesReceived.filter((a2) => a2).length === groupedRequests.length) {
  ...
  async get(relays, filter, params) {
    ...
    events.sort((a2, b) => b.created_at - a2.created_at);
...
function parseTLV(data) {
  ...
  while (rest2.length > 0) {
    ...
    let t = rest2[0];
    let l = rest2[1];
    let v = rest2.slice(2, 2 + l);
    rest2 = rest2.slice(2 + l);
    if (v.length < l)
...
function parse(event) {
  ...
  for (let i2 = event.tags.length - 1; i2 >= 0; i2--) {
    ...
    if (tag2[0] === "e" && tag2[1] && isHex32(tag2[1])) {
    ...
    if (tag2[0] === "q" && tag2[1] && isHex32(tag2[1])) {
    ...
    if (tag2[0] === "p" && tag2[1] && isHex32(tag2[1])) {
...
function unwrapManyEvents(wrappedEvents, recipientPrivateKey) {
  ...
  unwrappedEvents.sort((a2, b) => a2.created_at - b.created_at);
...
function parsePointer(tag2) {
  switch (tag2[0]) {
    case "E":
    case "e":
      if (!tag2[1] || !isHex32(tag2[1]))
        return void 0;
      return {
        id: tag2[1],
        relays: tag2[2] ? [tag2[2]] : [],
        author: tag2[3] && isHex32(tag2[3]) ? tag2[3] : void 0
      };
    case "A":
    case "a":
      if (!tag2[1])
...
function parse3(event) {
  ...
  for (const tag2 of event.tags) {
    ...
    if ((tag2[0] === "P" || tag2[0] === "p") && tag2[1] && isHex32(tag2[1])) {
      result.profiles.push({
        pubkey: tag2[1],
        relays: tag2[2] ? [tag2[2]] : []
      });
...
function makeZapRequest(params) {
  ...
  if ("event" in params) {
    ...
    if (isReplaceableKind(params.event.kind)) {
      const a2 = ["a", `${params.event.kind}:${params.event.pubkey}:`];
      zr.tags.push(a2);
    } else if (isAddressableKind(params.event.kind)) {
      let d = params.event.tags.find(([t, v]) => t === "d" && v);
      if (!d)
        throw new Error("d tag not found or is empty");
      const a2 = ["a", `${params.event.kind}:${params.event.pubkey}:${d[1]}`];
      zr.tags.push(a2);
    }
...
var NegentropyStorageVector = class {
  ...
  findLowerBound(begin, end, bound) {
    this._checkSealed();
    this._checkBounds(begin, end);
    return this._binarySearch(this.items, begin, end, (a2) => itemCompare(a2, bound) < 0);
  }
...
function compareUint8Array(a2, b) {
...
function itemCompare(a2, b) {
  if (a2.timestamp === b.timestamp) {
    return compareUint8Array(a2.id, b.id);
  }
  return a2.timestamp - b.timestamp;
}
...
function args(argv) {
  ...
  for (let i2 = 0; i2 < argv.length; i2++) {
    if (argv[i2].startsWith("--")) {
      const k = argv[i2].slice(2);
      const nxt = argv[i2 + 1];
      out[k] = nxt !== void 0 && !String(nxt).startsWith("--") ? argv[++i2] : "1";
    }
  }
  return out;
}
function readSecret(keyFile) {
  ...
  if (raw.startsWith("nsec1")) {
    const d = nip19_exports.decode(raw);
...
function toHex(input) {
  ...
  if (s.startsWith("npub1")) {
    const d = nip19_exports.decode(s);
...
function encryptFor(senderSk, recipientHex, obj) {
  const conv = nip44_exports.getConversationKey(senderSk, recipientHex);
  return nip44_exports.encrypt(JSON.stringify(obj), conv);
}
function decryptFrom(mySk, authorHex, payload) {
  const conv = nip44_exports.getConversationKey(mySk, String(authorHex).toLowerCase());
  return JSON.parse(nip44_exports.decrypt(payload, conv));
}
function isAllowedAuthor(authorHex, allowHexSet, myHex) {
  const a2 = String(authorHex || "").toLowerCase();
  if (!a2) return false;
  if (a2 === myHex) return true;
  return allowHexSet.has(a2);
}
function cmdNpubToHex(a2) {
  if (!a2.npub) throw new Error("--npub required");
  console.log(JSON.stringify({ hex: toHex(a2.npub) }));
}
function pubkeyHex(sk) {
  const pk = getPublicKey(sk);
  return (typeof pk === "string" ? pk : Buffer.from(pk).toString("hex")).toLowerCase();
}
function cmdEnsureKey(a2) {
  const keyFile = a2["key-file"];
  if (!keyFile) throw new Error("--key-file required");
  if (!existsSync(keyFile)) {
    mkdirSync(dirname(keyFile), { recursive: true });
    const sk2 = generateSecretKey();
    writeFileSync(keyFile, nip19_exports.nsecEncode(sk2) + "\n", { mode: 384 });
    try {
  ...
  console.log(JSON.stringify({ npub: nip19_exports.npubEncode(hex), hex }));
}
...
function toUnix(iso) {
  const t = Math.floor(new Date(iso).getTime() / 1e3);
  return Number.isFinite(t) ? t : Math.floor(Date.now() / 1e3);
}
async function cmdPublish(a2) {
  const keyFile = a2["key-file"];
  const relays = (a2.relays || DEFAULT_RELAYS.join(",")).split(",").map((s) => s.trim()).filter(Boolean);
  const job = JSON.parse(readFileSync(a2.in, "utf8"));
  if (!keyFile || !a2.in) throw new Error("--key-file and --in required");
  const sk = readSecret(keyFile);
  const myHex = pubkeyHex(sk);
  let given = parsePubkeyList(a2.recipients || "");
  const wantPublic = a2.public !== void 0;
...
async function cmdFetch(a2) {
  ...
  const mySk = readSecret(a2["key-file"]);
  const myHex = pubkeyHex(mySk);
  const allowHexSet = new Set(parsePubkeyList(a2["allow-list"] || ""));
  const relays = (a2.relays || DEFAULT_RELAYS.join(",")).split(",").map((s) => s.trim()).filter(Boolean);
  const since = Number(a2.since || 0);
  const limit = Math.min(500, Math.max(10, Number(a2.limit || 200)));
